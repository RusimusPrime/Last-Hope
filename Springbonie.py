...
class Bonnie(pygame.sprite.Sprite):
    def __init__(self, x, y, on_the_screen=True, *group):
        ...
        self.rect = pygame.transform.scale(pygame.image.load("SpringBonnie_sprites/1.png"), (128, 128)).get_rect()
        self.direction = "right"
        self.count = 0
        self.move_count = 0
        self.rect.x, self.rect.y = x, y
        self.size = (114, 200)
        self.current_image = pygame.transform.scale(pygame.image.load("SpringBonnie_sprites/1.png"), self.size)
        self.sprites_right = []
        self.sprites_left = []
        for i in range(1, 21):
            self.sprites_left += [pygame.transform.scale(pygame.image.load(f"SpringBonnie_sprites/{i}.png"), self.size)]

        for i in range(1, 21):
            self.sprites_right += [
                pygame.transform.flip(pygame.transform.scale(pygame.image.load(f"SpringBonnie_sprites/{i}.png"), self.size), True, False)]
